...
namespace Exoa.Designer
{
    public class AppController : MonoBehaviour
    {
        ...
        // IBMROS: undo/redo restore reuses the file-load path, which normally re-focuses
        // the camera 0.5s later. That refocus is right for opening a file but jarring for
        // an undo (it jolts the view). The undo service sets this before a restore; the
        // next OnFileLoaded skips its focus and clears the flag. An explicit flag is
        // robust where CancelInvoke("DelayedFocus") raced.
        public static bool SuppressNextFocusOnLoad;

        // IBMROS: A4 — statics survive disabled domain reload; reset per play session.
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
        private static void ResetStatics()
        {
            Instance = null;
            OnAppStateChange = null;
            SuppressNextFocusOnLoad = false;
        }
        ...
        {
            // IBMROS: undo/redo restore fires OnFileLoaded but must not jolt the camera.
            if (SuppressNextFocusOnLoad)
            {
                SuppressNextFocusOnLoad = false;
                return;
            }
