...
namespace Exoa.Designer
{
    public class UIBaseItem : MonoBehaviour
    {
        ...
        {
            IBMROS.Core.DocumentEvents.RaiseChanged(IBMROS.Core.DocumentChangeKind.Duplicate, "Duplicate"); // IBMROS: A1
        ...
        {
            //BroadcastChange(true);
            // IBMROS: A1 observable document — renaming was a silent mutation (this
            // handler was empty): saved data changed with no event, invisible to
            // undo/autosave. Announce it; no geometry rebuild is needed for a name.
            IBMROS.Core.DocumentEvents.RaiseChanged(IBMROS.Core.DocumentChangeKind.Rename, "Rename");
        }
        ...
        public void Delete()
        {
            IBMROS.Core.DocumentEvents.RaiseChanged(IBMROS.Core.DocumentChangeKind.Delete, "Delete"); // IBMROS: A1

            // IBMROS: A7 scoped invalidation — deleting an opening rebuilds only the
            // rooms whose walls it touched (+ building), not every room. Capture the
            // footprint before the control points are destroyed.
            bool isOpening = sequencingItemType == DataModel.FloorMapItemType.Door ||
                             sequencingItemType == DataModel.FloorMapItemType.Window ||
                             sequencingItemType == DataModel.FloorMapItemType.Opening;
            System.Collections.Generic.List<Vector3> footprint =
                cpc != null ? cpc.GetPointsWorldPositionList() : null;
            ...
            if (footprint == null)
                ...
                IBMROS.Core.ScopedRebuild.ForOpeningPositions(footprint);      // A7 stage 1
            else
                IBMROS.Core.ScopedRebuild.ForRoomRemoved(footprint);           // A7 stage 2
        ...
        virtual public DataModel.FloorMapItem GetData()
        {
            ...
            // IBMROS: A2 — every item carries a stable identity across save/load/undo.
