...
namespace Exoa.Designer
{
    public class ControlPointsController : MonoBehaviour
    {
        ...
        // IBMROS: openings are ONE item per placed point (each door/window its own item),
        // so undo is clean/blink-free and each opening is independently selectable. Set by
        // OpeningController.Init. Rooms keep multi-point (polygon) drawing.
        ...
        void Awake()
        {
            // IBMROS: fix color-shifting on load/undo. The color index came from
            // FindObjectsOfType<UIBaseItem>().Length — transient scene state that still
            // counts items pending deferred destruction, so every restore re-rolled all
            // floor colors (user-visible: floors flipping brown/green/yellow on undo).
            // The sibling index inside the freshly-created ControlPointsControllers
            // container is deterministic per document order instead.
            // (Long-term: persist a color index in the document — A2 schema note.)
            int count = transform.parent != null
                ? transform.parent.childCount - 1
                : GameObject.FindObjectsByType<UIBaseItem>().Length;
            int id = Mathf.Abs(count) % randomColors.Length;
        ...
        public Grid GetGrid()
        {
            ...
            // IBMROS: A4 — cached; was a per-call scene scan on a hot path.
            if (ibmrosCachedGrid == null)
                ibmrosCachedGrid = GameObject.FindAnyObjectByType<Grid>();
            return ibmrosCachedGrid;
        }
        private static Grid ibmrosCachedGrid; // IBMROS: A4
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (mouseHitSurface)
                {
                    ...
                    else if ((cp == null || cp == ghostPoint) && IsDrawing)
                    {
                        ...
                        //DragPoint(cp);
                        // IBMROS: one-opening-per-item. Placing a point completes THIS
                        // opening; stop drawing it and arm a fresh opening of the same
                        // type so each door/window is its own item (blink-free undo,
                        // independently selectable/deletable).
                        if (singlePointMode)
                        {
                            IsDrawing = false;
                            if (UIFloorMapMenu.instance != null)
                                UIFloorMapMenu.instance.ContinueDrawingNewOpening(this);
                        }
                    }
