    ...
    public class OpeningController : MonoBehaviour, IObjectDrawer
    {
        ...
        public void Init()
        {
            if (cpc != null)
            {
                cpc.drawPath = false;
                cpc.drawWidth = true;
                cpc.singlePointMode = true; // IBMROS: one opening per item (blink-free undo)
                cpc.Width = ui.Width;
                cpc.snapToGrid = true;
        ...
        // re-snap only the openings near a changed/removed room, instead of the global
        // OnRequestRepositionOpenings broadcast re-snapping every opening.
        ...
        private void OnControlPointsChanged()
        {
            ...
            IBMROS.Core.DocumentEvents.RaiseChanged(IBMROS.Core.DocumentChangeKind.Opening, "Move Opening"); // IBMROS: A1
            Rebuild();
        ...
        private void OnPathChanged()
        {
            ...
            IBMROS.Core.DocumentEvents.RaiseChanged(IBMROS.Core.DocumentChangeKind.Opening, "Place Opening"); // IBMROS: A1
            Rebuild();
        }
        ...
        {
            seq = s;
            IBMROS.Core.DocumentEvents.RaiseChanged(IBMROS.Core.DocumentChangeKind.ItemSettings, "Edit Opening"); // IBMROS: A1
            Rebuild();
        }
        ...
        private void Rebuild(bool sendRebuildRoomsEvent = true)
        {
            ...
            {
                // IBMROS: A7 — remove ONLY the excess trailing opening instances and keep
                // the rest. Was transform.ClearChildren(), which destroyed EVERY opening
                // in this item and recreated them — so reducing a multi-opening item's
                // point count (e.g. undoing the 3rd door) made the remaining doors blink.
                // Detach first so childCount reflects the removal immediately for the
                // reuse loop below.
                for (int k = transform.childCount - 1; k >= worldPosList.Count; k--)
                {
                    Transform excess = transform.GetChild(k);
                    excess.SetParent(null);
                    excess.gameObject.DestroyUniversal();
                }
            }
            ...
            if (sendRebuildRoomsEvent)
                // IBMROS: A7 scoped invalidation — an opening change rebuilds only the
                // rooms whose walls it touches (+ building), not every room. Was
                // OnRequestRebuildAllRooms (global broadcast → all rooms flicker).
                IBMROS.Core.ScopedRebuild.ForOpeningPositions(worldPosList);
